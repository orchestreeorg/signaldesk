...
- NEAR hourly: a dedicated bot (`NEAR_TELEGRAM_BOT_TOKEN`; chat falls back to `TELEGRAM_CHAT_ID`) gets the Position box every hour ART — live $NEAR, last lots, holdings USD, optional gpt-4o-mini NOTE. NEAR-titled desk RSS and dedicated NEAR feeds ping that same bot (`sendHeadline`). Not fusion. Mute and the FLASH cap do not apply.
